...
// Attribute helper now provided by attr!(key, value) macro from dot_generator, so the
// custom `make_attr` helper is no longer required.
    ...
    L: fmt::Debug + Eq + Hash + Serialize + for<'de> Deserialize<'de>,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for (label, targets) in self.graph.iter() {
