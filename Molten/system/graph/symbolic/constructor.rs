use attribute::{Assembler, Attribute, Category};
...
pub trait Constructor<Sink: attribute::Value> {
...
impl Transition for u8 {
    fn transition(self) -> Control {
        match self {
            b'[' => Control::Context(State::Initial),
            b'(' => Control::Group(State::Initial),
            b',' => Control::Partition,
            b'.' => Control::Continuation,
            b')' => Control::Group(State::Terminal),
            b']' => Control::Context(State::Terminal),
...
mod consume {
    ...
    pub fn space<Source: translator::traits::Translatable>(mut source: Source) -> Translation<u8> {
    ...
    pub fn next<Source: translator::traits::Translatable>(mut source: Source) -> Translation<u8> {
        Translation::rules()
            .limiter(1)
            .consume(source.by_ref())
...
impl<Source: translator::traits::Translatable> Constructor<String> for &mut Source {
    ...
    fn attribute(self) -> Result<Attribute<String>> {
        ...
        let mut assembler = Assembler::<String>::new(Category::Attribute);
        ...
        debug!("<=== Attribute");
        Ok(assembler.value(value.to_owned()).assemble())
    }
