    ...
    fn infer(&mut self, refractions: Relations<Waveform<T>>) -> Result<Inference> {
        ...
        for &label in &snapshot {
            ...
            for combination in ancestral.into_iter().multi_cartesian_product() {
                let combination_set = combination.into_iter().collect::<BTreeSet<Label>>();

                let labels = combination_set.iter().copied().collect::<Vec<_>>();
                ...
                let matchings = self.bipartite(combination_set.clone(), &relation.source)?;
                if matchings.count() == 0 {
                    continue;
                }

                for new_label in self.absorb(combination_set.clone(), relation.clone())? {
                    edges.insert(new_label);
                }
            }
        }
