...
pub fn span(span: wire::Span) -> Option<stream::Span> {
    ...
    let lifecycle = match span.event? {
        wire::span::Event::Begin(b) => stream::Lifecycle::Begin(stream::Begin {
            ...
            fields: b.fields.into_iter().filter_map(field).collect(),
        }),
    ...
    Some(stream::Span {
        ...
        channels: span.channels.into_iter().map(channel).collect(),
        lifecycle,
    })
}
...
pub fn event(event: wire::Event) -> Option<stream::Event> {
    Some(stream::Event {
        ...
        fields: event.fields.into_iter().filter_map(field).collect(),
    })
}
...
pub fn level(level: i32) -> stream::Level {
    match wire::Level::try_from(level).unwrap_or(wire::Level::Info) {
        wire::Level::Trace => stream::Level::Trace,
        wire::Level::Debug => stream::Level::Debug,
        wire::Level::Info => stream::Level::Info,
...
#[must_use]
pub fn channel(ch: wire::Channel) -> stream::Channel {
    stream::Channel {
        name: ch.name,
