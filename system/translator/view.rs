...
impl<Source: Read + Seek> Translator<Source, u8> for Source {
    ...
    fn view(
        ...
        let terminal =
            buffered.initial + u64::try_from(buffered.elements.len()).unwrap_or(u64::MAX);
        self.seek(SeekFrom::Start(buffered.initial))?;

        debug!("Source position reset to initial: {:?}", buffered.initial);
